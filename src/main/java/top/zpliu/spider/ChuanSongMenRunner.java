...
//@Component
public class ChuanSongMenRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        for (int navigationvalue = 3; navigationvalue < 5; navigationvalue += 1) {  //对应导航
    ...
    private void getListPage(String url, XSSFWorkbook wb, XSSFSheet se) throws InterruptedException {
        ...
        try {
            ...
            String cookie = "etime=2020-06-17+21%3A06%3A31%2B1week; ulevel=0; headimgurl=http%3A//thirdwx.qlogo.cn/mmopen/NLbUFBy5nMCw25vYicZtxZvia8nwQZNQOfclPbmVuNtdUbnQ23yTWD8ibkCWJdmsT09Vdhh5ticqG4Ml3O5lJqxzzBO2tWRl0Wvx/132; nickname=Alita; openid=obyEs1UJ9vkJtHLRRlkvoC6ceUOI";
...
/*        Element body2 = doc.body(); 暂时不取导航href
        ...
        Elements elements = body.getElementsByClass("title clear link_valid");  //列表循环   thumb-sm pull-left m-r-sm img-a
        for (Element element : elements) {
            ...
            List<Detail> details = getDetailPage(title.attr("href"), row,filename);        //获得每篇对应href去getDetailPage 获得每篇的信息 title.attr("href")
            filename++;
...
//            Thread.sleep(new Random().nextInt(1000) + 2000);
    ...
    private List<Detail> getDetailPage(String url, XSSFRow row,int filename) throws InterruptedException {
        ...
        Element body = doc.body();
        Elements content2 = body.getElementById("js_article").children(); //rich_media_content
        Elements imglink = content2.select("img[data-src]");
        ArrayList<String> imglist = new ArrayList<>();
        int i = 0;
        for (Element element2 : imglink) {
            String src = element2.attr("data-src");
            imglist.add(i, src);
            i++;
        }
        ...
        Elements content = body.getElementById("js_article").children();                             //rich_media_content
